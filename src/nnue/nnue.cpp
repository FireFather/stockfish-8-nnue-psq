...
static_assert(ft_out_dims % 64 == 0, "FtOutDims not a multiple of 64");
...
INLINE void affine_txfm(const int8_t* input, void* output, unsigned inDims, unsigned outDims, const int32_t* biases, const weight_t* weights,
	mask_t* inMask, mask_t* outMask, const bool pack8_and_calc_mask)
{
	assert(outDims == 32);
...
INLINE bool update_accumulator(position* pos)
{
	...
	for (unsigned i = 0; i < k_half_dimensions / TILE_HEIGHT; i++)
	{
		for (unsigned c = 0; c < 2; c++)
		{
			...
			{
				const auto prevAccTile = reinterpret_cast<vec16_t*>(&prev_acc->accumulation[c][i * TILE_HEIGHT]);
				for (unsigned j = 0; j < num_regs; j++)
					acc[j] = prevAccTile[j];
